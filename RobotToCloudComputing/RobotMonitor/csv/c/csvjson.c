...
#endif
{
	...
	for (size_t title_idx = 0; curr_pos < lexed_csv.tokens.size; ++curr_pos)
	{
		...
		{
			// Allow for incorrectly formed CSV (we get bad CSV from Spot) where every record has an extra,
			// empty field
			if (!title_idx && token->type == EMPTY && curr_pos + 1 < lexed_csv.tokens.size && dynarr_get(&lexed_csv.tokens, curr_pos + 1, Token).type == LINE_BREAK)
			{
				continue;
			}
...
LexedCsv _lex_csv(char *csv, size_t length)
{
	...
	// WARNING: Make sure error messages are well below 256 chars
